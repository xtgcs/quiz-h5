<template>
  <div>
        ...
        <div class="floor_bottom">
          ...
          <div v-if="gussDetail.status == 1">
            <countDown @endTime="endTime" :end="gussDetail.end_time"></countDown>
					...
					<div class="status_str" v-if="gussDetail.status != 1 && gussDetail.top_str">{{ gussDetail.top_str }}</div>
    ...
    <!-- footer -->
    <div v-if="gussDetail.is_click" class="confirm-btn" @click="confirmBtn(gussDetail.status, gussDetail.focus_id)">{{ gussDetail.bottom_str }}</div>
    <!-- 已经参与 -->
...
export default {
  ...
  methods: {
    ...
    confirmBtn(option, id) {
      if(option == 3) {
        this.$router.push({ path: "winning", query: {focus_id: id} });
        return
      }
